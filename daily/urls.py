from django.urls import path

from . import views


urlpatterns = [
    path('', views.index, name='index'),
    path('entry_1', views.entry_1, name='entry_1'),
    path('entry_2', views.entry_2, name='entry_2'),
    path('entry_3', views.entry_3, name='entry_3'),
    path('entry_4', views.entry_4, name='entry_4'),
    path('entry_5', views.entry_5, name='entry_5'),
	path('entry_6', views.entry_6, name='entry_6'),
	path('entry_7', views.entry_7, name='entry_7'),
	path('entry_8', views.entry_8, name='entry_8'),
	path('entry_9', views.entry_9, name='entry_9'),
	path('entry_10', views.entry_10, name='entry_10'),
	path('entry_11', views.entry_11, name='entry_11'),
	path('entry_12', views.entry_12, name='entry_12'),
	path('entry_13', views.entry_13, name='entry_13'),
	path('entry_14', views.entry_14, name='entry_14'),
	path('entry_15', views.entry_15, name='entry_15'),
	path('entry_16', views.entry_16, name='entry_16'),
	path('entry_17', views.entry_17, name='entry_17'),
	path('entry_18', views.entry_18, name='entry_18'),
	path('entry_19', views.entry_19, name='entry_19'),
	path('entry_20', views.entry_20, name='entry_20'),
	path('entry_21', views.entry_21, name='entry_21'),
	path('entry_22', views.entry_22, name='entry_22'),
	path('entry_23', views.entry_23, name='entry_23'),
	path('entry_24', views.entry_24, name='entry_24'),
	path('entry_25', views.entry_25, name='entry_25'),
	path('entry_26', views.entry_26, name='entry_26'),
	path('entry_27', views.entry_27, name='entry_27'),
	path('entry_28', views.entry_28, name='entry_28'),
	path('entry_29', views.entry_29, name='entry_29'),
	path('entry_30', views.entry_30, name='entry_30'),
	path('entry_31', views.entry_31, name='entry_31'),
	path('entry_32', views.entry_32, name='entry_32'),
	path('entry_33', views.entry_33, name='entry_33'),
	path('entry_34', views.entry_34, name='entry_34'),
	path('entry_35', views.entry_35, name='entry_35'),
	path('entry_36', views.entry_36, name='entry_36'),
	path('entry_37', views.entry_37, name='entry_37'),
	path('entry_38', views.entry_38, name='entry_38'),
	path('entry_39', views.entry_39, name='entry_39'),
	path('entry_40', views.entry_40, name='entry_40'),
	path('entry_41', views.entry_41, name='entry_41'),
	path('entry_42', views.entry_42, name='entry_42'),
	path('entry_43', views.entry_43, name='entry_43'),
	path('entry_44', views.entry_44, name='entry_44'),
	path('entry_45', views.entry_45, name='entry_45'),
	path('entry_46', views.entry_46, name='entry_46'),
	path('entry_47', views.entry_47, name='entry_47'),
	path('entry_48', views.entry_48, name='entry_48'),
	path('entry_49', views.entry_49, name='entry_49'),
	path('entry_50', views.entry_50, name='entry_50'),
	path('entry_51', views.entry_51, name='entry_51'),
	path('entry_52', views.entry_52, name='entry_52'),
	path('entry_53', views.entry_53, name='entry_53'),
	path('entry_54', views.entry_54, name='entry_54'),
	path('entry_55', views.entry_55, name='entry_55'),
	path('entry_56', views.entry_56, name='entry_56'),
	path('entry_57', views.entry_57, name='entry_57'),
	path('entry_58', views.entry_58, name='entry_58'),
	path('entry_59', views.entry_59, name='entry_59'),
	path('entry_60', views.entry_60, name='entry_60'),
	path('entry_61', views.entry_61, name='entry_61')
]
